...
class TestOneDriveNetflixBackend(unittest.TestCase):
    ...
    def test_files_endpoint_with_mock_auth(self, mock_get):
        ...
        test_client = httpx.AsyncClient(app=app, base_url="http://test")
    ...
    def test_files_all_endpoint_with_mock_auth(self, mock_get):
        ...
        # Now let's test the recursive folder traversal logic directly
        from backend.server import app
        ...
        async def mock_get_files_recursive(client, folder_id="root", folder_path=""):
            ...
            for file in files.get("value", []):
                ...
                if file.get("folder"):
                    ...
                    subfolder_files = await mock_get_files_recursive(client, file["id"], file_path)
        ...
        all_files = asyncio.run(mock_get_files_recursive(test_client))
    ...
    def test_video_format_support(self):
        ...
        # Extract the video extensions from the server code
        from backend.server import app
